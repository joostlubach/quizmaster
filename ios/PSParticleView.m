...
#import "PSParticleCell.h"

@interface PSParticleView ()

@property (nonatomic, readonly) CAEmitterLayer *emitterLayer;
@property (nonatomic) NSArray<CAEmitterCell*> *emitterCells;
@property (nonatomic) NSArray<NSDictionary*> *dynamicPropertiesCache;

@end

@implementation PSParticleView
...
- (void)setDynamicProperties:(NSArray<NSDictionary *> *)dynamicProperties {
  for (NSDictionary *property in dynamicProperties) {
    NSString *keyPath = [property valueForKey:@"keyPath"];
    id value = [property valueForKey:@"value"];
    
    [self.emitterLayer setValue:value forKeyPath:keyPath];
  }
  
  self.dynamicPropertiesCache = dynamicProperties;
}

- (void)insertSubview:(UIView *)view atIndex:(NSInteger)index
{
  if ([view isKindOfClass:[PSParticleCell class]]) {
    PSParticleCell *particleCell = (PSParticleCell*) view;
    ...
    particleCell.emitterSetupCompletion = ^(CAEmitterCell *cell) {
      dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [self setDynamicProperties:self.dynamicPropertiesCache];
